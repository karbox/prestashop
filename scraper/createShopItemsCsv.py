#uruchomienie skryptu zaklada istnienie pliku "albumsFinal.csv"
#wygenerowanego wczesniej za pomoca skryptu downloadCovers.py
...
with open("albumsFinal.csv", "r") as inputFile:
...
#Tworzymy dictionary genre/id
ids = []
startingId = categoryId
for i in range (0, len(uniqueGenres)):
    ids.append(startingId+i)

dictGenreId = dict(zip(uniqueGenres, ids))
...
#Product ID;Active (0/1);Name *;Categories (x,y,z...);Wholesale price;In store (0/1);Description
...
with open("albumsFinal.csv", 'r') as inputFile:
    with open ("products.csv", 'w') as outputFile:
        ...
        outputFile.write("Product ID;Active (0/1);Name *;Categories (x,y,z...);Unit price;In store (0/1);Description; \
                          URL rewritten;Available for order (0 = No, 1 = Yes);Show price (0 = No, 1 = Yes); \
                          Image URLs;Delete existing images (0 = No, 1 = Yes);Condition;Customizable (0 = No, 1 = Yes); \
                          Uploadable files (0 = No, 1 = Yes);Text fields (0 = No, 1 = Yes);Out of stock;ID / Name of shop;Advanced stock management;Depends On Stock;Quantity\n")
        ...
        for line in inputFile:
            ...
            name = artist + " - " + title
            category = dictGenreId[genre]
            unitPrice = random.randint(15, 45)
            urlRewritten = title.replace(' ', '').replace('/', '').replace('-', '').replace('&', '') #usuwanie niedozwolonych znakow

            #tworzenie opisu
            description = "Album pod tytulem " + title + " autorstwa " + artist + "." + " Uwazany za istotny dla gatunku " + genre + '.'

            #wpisujemy linie do pliku
            outputFile.write(str(productId) + s + active + s + name + s + str(category) + s + str(unitPrice) + s + inSale + s + description + s +
            urlRewritten + s + availableForOrder + s + showPrice + s + imgUrl + s + deleteExstImg + s + condition + s + customizable + s +
            uploadableFiles + s + textFields + s + outOfStock + s + IdOfShop + s + advStockMngmnt + s + dependsOnStock + s + quantity + '\n')
